...
struct ArtifactTemplateFactory {
    ...
    static func makeView(for record: DayRecord) -> some View {
        ...
        case .envelope: // The Royal Decree
            StyleRoyalDecreeView(record: record)
        case .vault: // The Classified
            StyleClassifiedView(record: record)
        case .pressedFlower: // The Botanist
            StyleBotanistView(record: record)
            
        // 🎬 Collection II: The Director (电影大师)
        case .filmNegative: // The Nolan
            StyleNolanView(record: record)
        case .postcard: // The Wes Anderson (Placeholder -> Postcard)
            StyleWesAndersonView(record: record)
        case .developedPhoto: // The Wong Kar-wai
            StyleWongKarWaiView(record: record)
            
        // 👠 Collection III: The Vogue (时尚女魔头)
        case .simple: // The Cover (Placeholder -> Simple)
            StyleVogueCoverView(record: record)
        case .polaroid: // The Polaroid SX-70
            StylePolaroidSX70View(record: record)
            
        // 🎫 Collection IV: The Voyager (环球旅行家)
        case .trainTicket: // The Boarding Pass (Placeholder -> TrainTicket)
            StyleBoardingPassView(record: record)
            
        // 💿 Collection V: The Collector (顶级藏家)
        case .vinylRecord: // The Vinyl
            StyleVinylView(record: record)
        case .movieTicket: // The Receipt (Placeholder -> MovieTicket)
            StyleReceiptView(record: record)
